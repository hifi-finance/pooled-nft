...
contract SetENSName_Test is ERC721Pool_Test {
    ...
    function test_RevertWhen_CallerIsNotFactory(address registrar, string memory name) external {
        //vm.expectRevert(IERC721Pool.ERC721Pool__CallerNotFactory.selector);
        vm.expectRevert(
    ...
    function testFuzz_SetENSName_Event(string memory name) external CallerIsFactory {
        vm.expectEmit({ checkTopic1: true, checkTopic2: true, checkTopic3: true, checkData: true });
        bytes32 nodeHash = erc721Pool.setENSName(address(reverseRegistrar), name);
        emit ENSNameSet(address(reverseRegistrar), name, nodeHash);
